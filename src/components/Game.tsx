      ...
      <div className="couples-list">
        ...
        <ul>
          {couples.map((couple, index) => (
            <li key={index}>{couple.person1} & {couple.person2}</li>
          ))}
        </ul>
      </div>

      <button onClick={createGame} disabled={!adminName || participants.length < 2}>
        Créer la partie
      </button>
...
export const JoinGame: React.FC = () => {
  ...
  const getParticipantStatus = (participantName: string) => {
    ...
    if (couple) {
      const partner = couple.person1 === participantName ? couple.person2 : couple.person1;
      ...
      {!hasJoined ? (
        ...
        <div className="draw-section">
          {!drawnName ? (
            ...
            <p className="drawn-name">Vous devez faire un cadeau à: {drawnName}</p>
          )}
        </div>
      )}
